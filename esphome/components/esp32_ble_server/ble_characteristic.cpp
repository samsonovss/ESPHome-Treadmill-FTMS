...
void BLECharacteristic::notify() {
  ...
  for (uint8_t i = 0; i < client_count; i++) {
    ...
    // FTMS Control Point responses must be confirmed indications. The ESP-IDF
    // GATT server handles the confirmation exchange; these responses are
    // serialized by the client, so no additional queue is required here.
