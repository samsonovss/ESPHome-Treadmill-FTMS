...
| Возможность | Zwift | Kinomap | FitShow | Kinni |
|---|---:|---:|---:|---:|
| Обнаружение дорожки по BLE | ✅ | ✅ | ✅ | ✅ |
| Получение скорости | ✅ | ✅ | ✅ | ✅ |
| Получение дистанции | ✅ | ✅ | ✅ | ✅ |
| Получение пульса | ✅ | ✅ | ✅ | ✅ |
| Старт / пауза / стоп из приложения | ✅ | ✅ | ✅ | ✅ |
| Изменение целевой скорости из приложения | ✅ | ✅ | ✅ | ✅ |
| Управление наклоном из приложения | ✅ | ✅ | ✅ | ✅ |
| Автоматическое управление наклоном по градиенту маршрута | ✅ | ✅ | ✅ | ✅ |
| Нативный автонаклон Zwift по градиенту маршрута | ✅ | ➖ | ➖ | ➖ |

✅ — проверено и работает; ➖ — неприменимо.

> Матрица отражает результаты практического тестирования текущей версии прошивки. Поведение может зависеть от версии приложения.

### FTMS и совместимость со Zwift

Для стандартных фитнес-приложений дорожка использует Bluetooth FTMS для передачи параметров тренировки и управления скоростью и наклоном.

Для нативного автонаклона в Zwift дополнительно реализован профиль совместимости Wahoo KICKR RUN. Zwift передаёт градиент маршрута через фирменный сервис Wahoo Run, а не через стандартную команду управления наклоном FTMS.

Таким образом:

- стандартный FTMS обеспечивает основную совместимость с Zwift, Kinomap, FitShow, Kinni и другими приложениями;
- профиль Wahoo KICKR RUN обеспечивает нативный автонаклон по градиенту маршрута в Zwift.
...
- измерение каденса.
- исследование возможности создания собственной нижней силовой платы управления двигателем и наклоном.
...
Проект развивается в свободное время и до сих пор финансировался из личных средств автора. Разработка прошивки и электроники, изготовление прототипов, закупка компонентов и испытания требуют значительных времени и затрат.

Если проект оказался вам полезен и вы хотите поддержать его дальнейшее развитие, любая помощь будет направлена на новые компоненты, прототипы, испытательное оборудование и развитие документации.
