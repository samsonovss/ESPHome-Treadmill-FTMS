...
Превратите старую беговую дорожку в современный умный тренажёр. Проект полностью заменяет штатный бортовой компьютер решением на базе ESP32-S3 и ESPHome, напрямую управляет нижней платой по UART и подключается к Zwift и другим фитнес-приложениям через Bluetooth FTMS без мостов и сторонних приложений. Он поддерживает нативное управление наклоном из Zwift, Home Assistant, дисплей Nextion, пульсовые программы, встроенные режимы тренировок и датчики безопасности. Текущая конфигурация разработана для плат семейства PSA(xx), но может быть адаптирована к другим дорожкам после исследования их протокола управления.
