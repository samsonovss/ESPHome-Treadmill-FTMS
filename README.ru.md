...
1. Скачайте репозиторий и откройте `esphome/config.yaml`.
2. Скопируйте `esphome/incline_profiles.h` рядом с YAML-файлом в каталог ESPHome.
3. Укажите Wi-Fi в `secrets.yaml`.
4. Замените установочные значения API, OTA, резервной точки доступа и MAC-адрес пульсометра.
