...
Превратите старую беговую дорожку в современный умный тренажёр. Проект полностью заменяет штатный бортовой компьютер решением на базе ESP32-S3 и ESPHome, напрямую управляет нижней платой по UART и подключается к Zwift и другим фитнес-приложениям через Bluetooth [Fitness Machine Service (FTMS)](docs/specs/FTMS_v1.0.pdf) без мостов и сторонних приложений. Он поддерживает нативное управление наклоном из Zwift, Home Assistant, дисплей Nextion, пульсовые программы, встроенные режимы тренировок, а также определение положения пользователя и безопасной зоны с помощью лазерного ToF-датчика расстояния VL53L1X. Текущая конфигурация разработана для плат семейства PSA(xx), но может быть адаптирована к другим дорожкам после исследования их протокола управления.

https://github.com/user-attachments/assets/f829769c-47f4-4251-8f99-d2f9a6cf358a

## Ключевые возможности

- **Прямое подключение по Bluetooth FTMS** - без мостов и сторонних приложений;
- **Нативное управление наклоном из Zwift** - градиент трассы физически управляет дорожкой;
- **Интеграция с Home Assistant**;
- **Собственные сценарии тренировок в Home Assistant** - создавайте и запускайте программы с автоматическим изменением скорости и наклона;
- **Сенсорный интерфейс Nextion**;
- **Пульсовые программы, HIIT, разминка и заминка**;
- **ESPHome и полностью локальная работа** - облако не требуется;
- **Лазерный ToF-контроль безопасной зоны, оптическая обратная связь скорости и мониторинг температуры двигателя**.
...
> [!IMPORTANT]
> Исследование протокола управления обычно является самой сложной частью адаптации проекта к другой плате беговой дорожки.
...
Доступны настраиваемые профили поведения дорожки:

- **Коррекция скорости:** `Soft`, `Precise`, `Aggressive` — определяют, насколько активно система компенсирует отклонение фактической скорости полотна от заданной;
- **Разгон и замедление:** `Soft`, `Normal`, `Fast`, `Technogym` — определяют плавность и скорость изменения скорости полотна.

## Интерфейсы

Локальный веб-интерфейс ESPHome доступен на порту `80` без авторизации.
...
- [`esphome/packages/`](esphome/packages/) — оборудование, глобальные переменные, скрипты, сенсоры, BLE/FTMS, элементы управления и Nextion;
