...
> [!WARNING]
> Проект напрямую управляет двигателем и механизмом наклона. Перед первым запуском проверьте аварийную остановку, пределы скорости и наклона, UART-команды и температурную защиту. Не испытывайте новую конфигурацию, находясь на движущемся полотне.
...
4. Проверьте распиновку, UART-параметры и команды своей дорожки.
5. Отключите силовую часть и сначала проверьте загрузку ESP32, Nextion и датчики.
6. Создайте резервную копию, скомпилируйте конфигурацию и изучите логи.
7. Проверьте остановку, минимальную скорость и возврат наклона без человека на полотне.
...
- [`esphome/`](esphome/) — конфигурация ESPHome и профили наклона;
...
- веб-интерфейс без Home Assistant;
- MQTT-интеграция;
