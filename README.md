...
## О проекте и как это работает
ESPHomeTreadmill превращает обычную беговую дорожку в современное умное устройство с поддержкой популярных фитнес-платформ вроде Zwift и интеллектуальными функциями. Проект построен на базе ESP32 S3, который через UART общается с платой дорожки — например, с серией PSA(xx). Команды вроде `[Setspeed:010]` (1 км/ч) или `[Setincline:000]` (0%) были обнаружены путём анализа трафика между платой и дорожкой с помощью логгера UART. Эти данные микроконтроллер обрабатывает, преобразует в реальные значения скорости и наклона, а затем передаёт по Bluetooth Low Energy (BLE) в приложения или сохраняет локально для анализа в Grafana.

Пульсометр подключается отдельно через BLE, предоставляя данные о частоте сердечных сокращений. Интеллектуальные алгоритмы в реальном времени анализируют пульс и, при необходимости, плавно корректируют скорость или наклон дорожки, чтобы поддерживать заданную зону тренировки. Например, если ваш пульс выходит за пределы целевой зоны, ESP32 S3 автоматически изменяет параметры, делая тренировку персонализированной и эффективной. Это создаёт полноценную систему для умного бега с минимумом затрат.
