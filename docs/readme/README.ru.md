...
- **FC33**: Оптический датчик скорости для калибровки беговой дорожки. (но не обязательно)
- **Дисплей Nextion**: для улучшенного пользовательского интерфейса и взаимодействия.
- **Беговая дорожка**: идеально — с платой **[PSA(xx)](images/PSA(XX)H.jpg)**, но подойдёт любая с UART (RX-TX).
...
- **Поддержка Zwift**: Полная интеграция с популярной платформой.
  ![ESPHome Treadmill Zwift](images/Zwift.gif)
...
### Интерфейс в Hassio
![Hassio Interface](images/hassio.png)
...
- 27 мая 2025:
  - Добавлен расчёт MET и VO2 с поддержкой интервальных тренировок для более точного анализа энергозатрат и аэробной мощности.
  - Реализован вывод результатов тренировки на дисплей Nextion после завершения, включая длительность, дистанцию, калории, среднюю скорость, наклон, MET, VO2, пульс и время в зонах пульса.
  - Добавлен подробный лог результатов тренировки в формате "Workout Summary" с данными о пользователе (пол, возраст, вес), тренировке (длительность, дистанция, калории, скорость, наклон, MET, VO2) и зонах пульса.
  - Обновлён интерфейс дисплея Nextion с улучшенным дизайном и добавлением поля для ввода возраста пользователя, используемого для более точного расчёта килокалорий.
  - Переработана клавиатура Nextion для защиты от некорректного ввода, повышая надёжность взаимодействия с пользователем.
    
- 26 мая 2025:
  - Исправлен автоматический перезапуск беговой дорожки QZ Fitness при включённом управлении наклоном для Zwift путём добавления флага `manual_stop` в `stop_program` и блокировки команд "Start" (0x07) в `ftms_control_point_char`, если `manual_stop=true`. В обычных режимах проблем не наблюдалось.
  - Добавлен скрипт `reset_manual_stop` для сброса флага `manual_stop` через 5 секунд, что позволяет другим FTMS-приложениям (например, Zwift, Qdomyos-Zwift) запускать дорожку после ручной остановки.
  - Предотвращены нежелательные команды наклона от QZ Fitness после остановки в режиме управления наклоном для Zwift путём отклонения команд "Set Incline" (0x03) при `motor_running=false` в `ftms_control_point_char`.
  - Оптимизирована последовательность `stop_program`: уведомления FTMS ("Stopped or Paused" и "Idle") и команды UART (`[SETSPD:000]`) отправляются до сброса `motor_running` и установки `manual_stop`, улучшая синхронизацию с QZ Fitness в режиме управления наклоном для Zwift.
