...
# Трансформируйте вашу беговую дорожку с FTMS на ESPHome через ESP32-S3 BLE с дисплеем Nextion
...
## О проекте
Превратите старую беговую дорожку в умного спутника для тренировок! С помощью ESP32-S3 и прошивки ESPHome вы можете добавить поддержку FTMS для фитнес-приложений через BLE, а также интеллектуальные программы на основе пульса и встроенные режимы тренировок. Проект разработан для дорожек с платами PSA(xx), но адаптируется к любой дорожке с поддержкой UART. Минимальные затраты, максимальный потенциал!

## Как это работает
Проект использует ESP32-S3 для связи с платой дорожки (например, PSA(xx)) через UART. Команды, такие как `[SETSPD:010]` (1 км/ч) или `[SETINC:000]` (0%), были восстановлены путём анализа трафика с помощью UART-логгера. Микроконтроллер обрабатывает данные, преобразует их в реальные значения скорости и наклона и передаёт через Bluetooth Low Energy (BLE) в приложения, такие как Zwift, или сохраняет локально для анализа в Grafana.

Пульсометр подключается через BLE для передачи данных о пульсе. Интеллектуальные алгоритмы в реальном времени анализируют пульс и плавно регулируют настройки дорожки, чтобы поддерживать целевую тренировочную зону. Например, если пульс выходит за пределы цели, скорость автоматически корректируется для персонализированной и эффективной тренировки.

### Преимущества
- **Гибкость**: Совместимость с любой дорожкой, поддерживающей UART.
- **Современность**: Работает на мощном микроконтроллере ESP32-S3.
- **Доступность**: Требует минимального набора компонентов.
...
- **Мониторинг пульса**: Если пульс не достигает Зоны 1 (например, 91 уд/мин) в течение заданного времени, система отображает сообщение о статусе, например: «Ожидание пульса 91».
- **Динамическое ускорение**: Если пульс остаётся ниже Зоны 1, скорость увеличивается на 0.5 км/ч с плавным нарастанием:
 - Добавляется 0.1 км/ч каждые 2 секунды, пока не будет достигнута полная корректировка 0.5 км/ч, обеспечивая комфортный переход.
 - После достижения 0.5 км/ч система делает паузу на 10 секунд, повторно проверяет пульс и при необходимости повторяет увеличение на 0.5 км/ч.
